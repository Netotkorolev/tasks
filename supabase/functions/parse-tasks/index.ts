...
- Исключение: конструкция "Главное действие: шаг1, шаг2, шаг3" (особенно "Сделать X:", "Подготовить X:", "Организовать X:", "Запустить X:", "Разобрать X:") — это ОДНА задача, а не несколько. Часть до двоеточия — text задачи, перечисленные через запятую/точку с запятой части после двоеточия — это subtasks (раздел 12), а не отдельные задачи.
  Пример: "Сделать презентацию для Зимина: собрать цифры, написать структуру, сделать дизайн, отправить в пятницу" → ОДНА задача text="Сделать презентацию для Зимина", subtasks=["Собрать цифры","Написать структуру","Сделать дизайн","Отправить презентацию Зимину"], НЕ четыре отдельные задачи.
...
- в тексте есть конструкция "Главное действие: шаг1, шаг2, шаг3" (см. раздел 1) — части после двоеточия, перечисленные через запятую/точку с запятой, это шаги, а не отдельные задачи;
- пользователь сам перечислил шаги другим способом ("сначала... потом... и в конце...", "1) ... 2) ... 3) ...");
...
Если шагов 3 или больше — ставь effort="elephant" родительской задаче (явный признак многоэтапной задачи), кроме случаев, когда сами шаги тривиальны (на уровне 2m/5m каждый).
Каждый шаг — короткое самостоятельное действие, без "надо бы"/эмоций, в том же стиле, что text задачи.
ДАТА — определи, к чему она относится:
- если дата стоит до двоеточия или явно относится ко всей задаче целиком ("сделать презентацию до пятницы: собрать цифры, ...") — due_date ставится родительской задаче, у всех subtasks due_date=null;
- если дата привязана к конкретному шагу после двоеточия ("..., отправить в пятницу") — due_date ставится только этому шагу; у родительской задачи и у остальных шагов due_date=null, если для них дата не названа отдельно.
Если дата не названа явно ни для задачи, ни для конкретного шага — due_date=null. Не выводи дату шага из даты родительской задачи и не пытайся вычислить её сам.
